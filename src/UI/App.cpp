...
App::App() { LOG_DEBUG("App", "App constructor called"); }
...
App::~App() {
  ...
  if (m_workerLLM)
    m_workerLLM->GetCancellationToken().Cancel();
  ...
  auto joinFuture = [](std::future<void> &f, const char *name) {
    if (f.valid()) {
      ...
      if (status == std::future_status::ready) {
        ...
        LOG_WARN("App", std::string("~App: ") + name +
                            " did not finish in 5s — detaching");
      }
    }
  };
  ...
  // Release references to ensure SQLite closes the file
  m_docService.reset();
  m_db.reset();

  // Wipe the database file on exit to ensure clean state
...
bool App::Initialize(const std::filesystem::path& dataDir, bool freshSession) {
  ...
  // Indexer + Repository
  auto indexer =
      std::make_shared<Infrastructure::SqliteFts5Indexer>(m_db, m_embedder);
  ...
  LOG_INFO("App",
           "Dual LlamaCppProvider instances created (Worker + Reasoner)");
  ...
  LOG_INFO("App",
           "Worker model path:   " + workerPath.string() + " (exists: " +
               std::string(std::filesystem::exists(workerPath) ? "YES" : "NO") +
               ")");
  LOG_INFO(
      "App",
      "Reasoner model path: " + reasonerPath.string() + " (exists: " +
          std::string(std::filesystem::exists(reasonerPath) ? "YES" : "NO") +
          ")");
  ...
  if (std::filesystem::exists(reasonerPath)) {
    ...
    reasonerCfg.params.contextWindowSize =
        8192; // Doubled: more passages in context, longer answers
    reasonerCfg.params.temperature =
        0.3f; // 0.3 for explanatory synthesis (0.0 causes robotic output)
  ...
  // Load initial state
  RefreshDocumentList();
  LOG_INFO("App", "Document list loaded: " +
                      std::to_string(m_state.documents.size()) + " docs");
...
void App::AutoDetectAndLoadModels() {
  ...
  if (!reasonerPath.empty()) {
    ...
    m_state.reasonerStatus =
        "Auto-loading: " + reasonerPath.filename().string();
    LOG_INFO("App",
             "Auto-loading Reasoner: " + reasonerPath.filename().string());
  ...
  if (std::filesystem::exists(embedPath)) {
    ...
    auto result = m_embedder->LoadModel(embedPath);
    if (result) {
      m_state.embedStatus = "Ready";
      m_state.embedLoaded = true;
      LOG_INFO("App", "Loaded embedding model: " + m_state.embedName);
    } else {
      m_state.embedStatus = "Load failed: " + result.error();
      m_state.embedLoaded = false;
      LOG_ERROR("App", "Failed to load embedding model: " + result.error());
    }
  } else {
...
void App::IngestDocument(const std::filesystem::path &filePath) {
  ...
  m_state.lastIngestionError =
      ""; // Clear previous error so banner doesn't show during new ingestion

  m_ingestionFuture = std::async(std::launch::async, [this, filePath]() {
    try {
      LOG_INFO("App", "[async] Ingestion thread started for: " +
                          filePath.filename().string());
      auto result = m_docService->IngestDocument(
          filePath, [this](const Application::IngestionProgress &p) {
      ...
      if (result) {
        ...
        m_pendingAutoQuery = "Summarize in bullet points: " + result->filename;
      } else {
        // Store error persistently so the UI shows it even after
        // ingesting=false
      ...
      LOG_ERROR("App",
                "[async] Exception in Ingestion: " + std::string(e.what()));
    } catch (...) {
      std::lock_guard lock(m_stateMutex);
...
void App::AskQuestion(const std::string &query) {
  if (m_state.generating || query.empty()) {
    LOG_WARN("App",
             "AskQuestion skipped — generating=" +
  ...
  bool isAutoSummary = (query.find("Give me a comprehensive summary of ") == 0);
  m_state.chatHistory.push_back(
      {ChatMessage::Role::User, query, {}, 0, 0, isAutoSummary});

  m_state.generating = true;
  m_state.generationCancelled = false;
  ...
  m_generationFuture = std::async(std::launch::async, [this, query]() {
    try {
      ...
      auto result = m_inferService->AskQuestionStreaming(
          query,
          ...
          [this](const Application::AnswerProgress &p) {
            ...
            m_state.lastActivityTime = std::chrono::steady_clock::now();
            switch (p.stage) {
            case Application::AnswerProgress::Stage::Searching:
              m_state.generationStatus = "Searching documents...";
              break;
            case Application::AnswerProgress::Stage::Assembling:
              m_state.generationStatus = "Assembling context...";
              break;
            case Application::AnswerProgress::Stage::Generating:
              m_state.generationStatus = "Generating answer...";
              break;
            case Application::AnswerProgress::Stage::Validating:
              m_state.generationStatus = "Validating citations...";
              break;
            case Application::AnswerProgress::Stage::Complete:
              m_state.generationStatus = "Complete";
              break;
            }
          });
      ...
      std::lock_guard lock(m_stateMutex);
      if (result) {
        m_state.chatHistory.push_back({ChatMessage::Role::Assistant,
                                       result->answerText, result->citations,
                                       result->generationTimeMs,
                                       result->sectionsSearched});
      } else {
        ...
        if (wasCancelled) {
          m_state.chatHistory.push_back(
              {ChatMessage::Role::System, "Operation cancelled.", {}, 0, 0});
          LOG_INFO("App", "[async] Generation cancelled by user/watchdog");
        } else {
          m_state.chatHistory.push_back(
              {ChatMessage::Role::System, "Error: " + err, {}, 0, 0});
          LOG_ERROR("App", "[async] Generation error: " + err);
        }
      }
    } catch (const std::exception &e) {
      std::lock_guard lock(m_stateMutex);
      m_state.chatHistory.push_back({ChatMessage::Role::System,
...
void App::LoadReasonerModel(const std::filesystem::path &modelPath) {
  ...
  m_reasonerLoadFuture = std::async(std::launch::async, [this, modelPath]() {
    LOG_INFO("App", "[async] Reasoner load thread started: " +
    ...
    // We MUST keep Flash Attention enabled for speed, but use F16 KV cache to
    // prevent the crash.
    ...
    if (lowerPath.find("phi") != std::string::npos) {
      ...
      LOG_INFO("App", "Detected Phi model — set KV cache to F16 to prevent "
                      "crashes, keeping Flash Attention enabled for speed.");
    }
...
void App::LoadWorkerModel(const std::filesystem::path &modelPath) {
  ...
  m_workerLoadFuture = std::async(std::launch::async, [this, modelPath]() {
    LOG_INFO("App", "[async] Worker load thread started: " +
                        modelPath.filename().string());
    Core::ModelParams params;
...
void App::Update() {
  ...
  // ─── Auto Query Trigger ───
  if (!m_pendingAutoQuery.empty() && !m_state.ingesting &&
      !m_state.generating) {
    std::string query = m_pendingAutoQuery;
    m_pendingAutoQuery.clear();
    AskQuestion(query);
  }
  ...
  if (m_state.generating) {
    ...
    m_state.elapsedGenerationSec =
        std::chrono::duration<double>(now - m_state.generationStartTime)
            .count();
    ...
    if (evalTotal > 0 && evalDone < evalTotal) {
      ...
      std::lock_guard lock(m_stateMutex);
      m_state.generationStatus =
          "Evaluating context... " + std::to_string(evalDone) + "/" +
          std::to_string(evalTotal) + " tokens (" + std::to_string(pct) + "%)";
    }
    ...
    if (heartbeatMs > 0) {
      ...
      if (silentMs > 600'000) { // 10 minutes — thread is truly dead
        LOG_ERROR(
            "App",
            "Heartbeat watchdog: inference thread unresponsive for >10 min. "
...
void App::CancelGeneration() {
  ...
  {
    std::lock_guard lock(m_stateMutex);
    m_state.generationCancelled = true;
...
std::vector<std::string> App::ScanAvailableModels() const {
  ...
  if (!std::filesystem::exists(m_modelsDir)) {
    LOG_WARN("App",
             "ScanAvailableModels: dir not found: " + m_modelsDir.string());
    return models;
  }
  ...
  LOG_INFO("App", "ScanAvailableModels: found " +
                      std::to_string(models.size()) + " .gguf files");
  ...
  std::sort(models.begin(), models.end(),
            [](const std::string &a, const std::string &b) {
              ...
              auto priority = [](const std::string &s) -> int {
                ...
                if (lower.find("gemma-3-12") != std::string::npos)
                  return 1;
                // Gemma-3-4B
                if (lower.find("gemma-3-4") != std::string::npos)
                  return 2;
...
void App::RefreshDocumentList() {
  ...
  LOG_DEBUG("App", "RefreshDocumentList: " +
                       std::to_string(m_state.documents.size()) + " docs");
}
